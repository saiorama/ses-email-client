...
const DELIM = '/';
const BOOKMARKS_EMAIL_PREFIX = `bookmarks@`;
const FROM_EMAIL_PREFIX = `Sai <sai@`;
const REPLY_TO_EMAIL_PREFIX = `no-reply@`;
const TEMPLATE_NAME = `email-was-bookmarked-template`;
...
let AWS_ACCESS_KEY_ID="AKIA4R7E54H5KVO5CBTB";
let AWS_SECRET_ACCESS_KEY="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";
let AWS_DEFAULT_REGION="ap-south-1";
const ses = new AWS.SES({region: 'us-east-1', accessKeyId: AWS_ACCESS_KEY_ID, secretAccessKey: AWS_SECRET_ACCESS_KEY});
...
exports.handler = async (event) => {
    ...
    if(awsSnsRecord && awsSnsRecord.Sns && awsSnsRecord.Sns.Message){
        ...
        let axiosConfig = {
            "url": `https://api.zeer0.com/v001/get?domain=${domain}&id=${emailId}`
        };
        ...
        .then(async r => {
            ...
            console.log(`ec.to => `, emailContent.to, 'ec.cc', emailContent.cc);
